# Synq 公开仓库截图刷新计划

日期：2026-08-20
状态：已完成
目标：新增可复现的公开截图脚本，用真实安装版 Synq 的匿名演示模式刷新 README 的桌面与手机截图，并创建一个不混入既有工作树改动的独立 commit。

## 范围与已核对接口

- README 仅引用 `screenshots/claude-grok-quota-desktop.png` 与 `screenshots/claude-grok-quota-mobile.png`（`README.md:9-11`）。
- 首次引导的演示入口是 `查看演示`；其回调调用 `setDemoMode(true)` 与 `setOnboardingComplete(true)`（`src/components/synq/onboarding.tsx:97-108`、`src/routes/index.tsx:66-78`）。
- 演示数据由 `resetDemo()` 调用 `seedHistory(now)` 与 `startTrio(now)` 生成（`src/lib/quota/store.ts:507-529`）。
- Playwright context、诊断监听和截图 API 已按 `scripts/onboarding-e2e.mjs:90-127,380-426` 核对；输出路径继续经过 `scripts/browser-guard.mjs` 限制。
- 本次只改 plan、截图脚本、`package.json` 和 README 实际引用的两张图片。当前 modified 的 `synq-mobile.png`、`synq-plugin.png`、`synq-report.png` 及其他 untracked 文件均属于用户工作树，不暂存。

## Step 1：以失败断言锁定截图尚未刷新

先确认 index 为空，避免把用户已有 staged 内容混入后续 commit：

```bash
if ! git diff --cached --quiet; then
  echo "FAIL: git index already contains staged changes"
  exit 1
fi
```

再运行红灯断言；当前两张图与 `HEAD` 相同，因此命令必须非零退出：

```bash
if git diff HEAD --quiet -- \
  screenshots/claude-grok-quota-desktop.png \
  screenshots/claude-grok-quota-mobile.png; then
  echo "FAIL: public screenshots are still identical to HEAD"
  exit 1
fi
```

验收：红灯确实失败，证明后续绿灯来自新截图，而不是沿用旧图片。

## Step 2：新增可复现的公开截图脚本并生成两张新图

在 `package.json` 增加：

```json
"screenshots:public": "node scripts/capture-public-screenshots.mjs"
```

新增 `scripts/capture-public-screenshots.mjs`，完整实现如下：

```js
#!/usr/bin/env node
import assert from "node:assert/strict";
import { existsSync, mkdirSync, readFileSync, readdirSync, writeFileSync } from "node:fs";
import { dirname, resolve } from "node:path";
import { fileURLToPath } from "node:url";
import { chromium } from "playwright";
import { toCrossJSON } from "seroval";
import { checkedOutputPath, checkedUrl } from "./browser-guard.mjs";

for (const key of [
  "BROWSER_ALLOW_EXTERNAL_HOST",
  "http_proxy",
  "https_proxy",
  "HTTP_PROXY",
  "HTTPS_PROXY",
  "ALL_PROXY",
  "all_proxy",
]) {
  delete process.env[key];
}
process.env.NO_PROXY = "*";
process.env.no_proxy = "*";

const ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const BASE = checkedUrl(process.argv[2] || "http://127.0.0.1:4780/");
const OUTPUT_ROOTS = [resolve(ROOT, "screenshots"), resolve("/tmp")];
const OUTPUT_DIR = checkedOutputPath(
  resolve(process.argv[3] || resolve(ROOT, "screenshots")),
  OUTPUT_ROOTS,
);
const SPECS = [
  {
    name: "desktop",
    viewport: { width: 1280, height: 900 },
    file: "claude-grok-quota-desktop.png",
    maxHeight: 5000,
  },
  {
    name: "mobile",
    viewport: { width: 390, height: 844 },
    file: "claude-grok-quota-mobile.png",
    maxHeight: 12000,
  },
];
const EXPECTED_README_IMAGES = SPECS.map((spec) => `screenshots/${spec.file}`);
const DEV_AVAILABILITY_ID = "cHVsbEFnZW50QXZhaWxhYmlsaXR5";
const FIVE_HOUR_MS = 5 * 60 * 60 * 1000;
const WEEK_MS = 7 * 24 * 60 * 60 * 1000;
const fetchedAt = Date.now();
const publicSlice = (agent, windowPct, weekPct, planLabel, source, windowKind) => ({
  agent,
  windowPct,
  weekPct,
  windowResetsAt: null,
  weekResetsAt: null,
  weekStartedAt: null,
  windowDurationMs: windowKind === "five_hour" ? FIVE_HOUR_MS : WEEK_MS,
  weekDurationMs: WEEK_MS,
  burnPctPerHour: 0,
  planLabel,
  products: [],
  prepaidBalance: null,
  onDemandUsed: null,
  onDemandCap: null,
  source,
  fetchedAt,
  windowKind,
});
const publicPersistedState = {
  state: {
    onboardingComplete: true,
    demoMode: true,
    adapterHint: true,
    agentAvailability: { claude: true, grok: true, codex: true },
    captureEnabled: { claude: true, grok: true, codex: true },
    claudePlanId: "claude-max-20x",
    grokPlanId: "grok-super",
    codexPlanId: "chatgpt-plus",
    official: {
      claude: {
        ...publicSlice("claude", 24, 34, "Claude Max", "oauth-usage", "five_hour"),
        modelWeekLimits: { fable: { usedPct: 26, resetsAt: null } },
      },
      grok: publicSlice("grok", 37, 37, "SuperGrok", "billing-api", "weekly"),
      codex: publicSlice("codex", 32, 32, "ChatGPT Plus", "wham-usage", "weekly"),
    },
  },
  version: 0,
};
const SENSITIVE = [
  /\/Users\//i,
  /\/Volumes\//i,
  /\/home\//i,
  /[A-Z]:[\\/]+Users[\\/]+/i,
  /\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/i,
  /\b[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\b/i,
  /\b(?:sk|ghp|github_pat)_[A-Za-z0-9_-]{12,}\b/i,
  /AI_Agent_MultiTenant_Workstation|simverse-world|deepseek-harness/i,
];

function pngSize(path) {
  const png = readFileSync(path);
  assert.equal(png.subarray(1, 4).toString("ascii"), "PNG", `${path} is not PNG`);
  return { width: png.readUInt32BE(16), height: png.readUInt32BE(20) };
}

function assertReadmeContract() {
  const readme = readFileSync(resolve(ROOT, "README.md"), "utf8");
  const images = [...readme.matchAll(/src="\.\/(screenshots\/[^"?]+\.png)"/g)].map(
    (match) => match[1],
  );
  assert.deepEqual(images, EXPECTED_README_IMAGES, "README public image set changed");
  assert.ok(!readme.includes("claude-grok-quota-production.png"));
}

function assertPrivateTextAbsent(surface) {
  for (const pattern of SENSITIVE) assert.doesNotMatch(surface, pattern);
}

function productionServerFnIds(name) {
  const ids = [];
  for (const directory of [
    resolve(ROOT, ".output/server/_ssr"),
    resolve(ROOT, ".vercel/output/functions/__server.func/_ssr"),
    "/Applications/Synq.app/Contents/Resources/synq-server/server/_ssr",
  ]) {
    if (!existsSync(directory)) continue;
    for (const file of readdirSync(directory).filter(
      (entry) => entry.startsWith("routes-") && entry.endsWith(".mjs"),
    )) {
      const source = readFileSync(resolve(directory, file), "utf8");
      const match = source.match(
        new RegExp(`var ${name} = [^;]+createSsrRpc\\("([a-f0-9]{64})"\\)`),
      );
      if (match?.[1]) ids.push(match[1]);
    }
  }
  return ids;
}

const AVAILABILITY_IDS = new Set([
  DEV_AVAILABILITY_ID,
  ...productionServerFnIds("pullAgentAvailability"),
]);

function isAvailabilityRequest(request) {
  if (request.method() !== "GET") return false;
  try {
    const pathname = new URL(request.url()).pathname;
    const prefix = "/_serverFn/";
    if (!pathname.startsWith(prefix)) return false;
    const encoded = pathname.slice(prefix.length).split("/")[0];
    const id = decodeURIComponent(encoded);
    if (AVAILABILITY_IDS.has(id)) return true;
    return Buffer.from(id, "base64url").toString("utf8") === "pullAgentAvailability";
  } catch {
    return false;
  }
}

function cardAround(page, heading) {
  return page
    .getByRole("heading", { name: heading, exact: true })
    .locator("xpath=ancestor::div[contains(@class,'rounded-2xl')][1]");
}

async function capture(browser, spec) {
  const context = await browser.newContext({ viewport: spec.viewport, locale: "zh-CN" });
  await context.addInitScript((persistedState) => {
    localStorage.setItem("synq-quota-v8", JSON.stringify(persistedState));
    sessionStorage.clear();
  }, publicPersistedState);
  await context.route(/https:\/\/(grok\.com|fonts\.(gstatic|googleapis)\.com)\//, async (route) => {
    const type = route.request().resourceType();
    await route.fulfill({
      status: 200,
      contentType:
        type === "script"
          ? "application/javascript"
          : type === "stylesheet"
            ? "text/css"
            : "application/octet-stream",
      body: "",
    });
  });
  let availabilityIntercepted = false;
  await context.route("**/_serverFn/**", async (route) => {
    const request = route.request();
    if (!availabilityIntercepted && isAvailabilityRequest(request)) {
      availabilityIntercepted = true;
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        headers: { "x-tss-serialized": "true" },
        body: JSON.stringify(
          toCrossJSON({
            result: { claude: true, grok: true, codex: true },
            error: undefined,
            context: {},
          }),
        ),
      });
      return;
    }
    await route.continue();
  });
  const errors = { console: [], page: [], request: [], http: [] };
  const networkSurface = [];
  const page = await context.newPage();
  page.setDefaultTimeout(20_000);
  page.setDefaultNavigationTimeout(20_000);
  page.on("console", (message) => {
    if (message.type() === "error") errors.console.push(message.text());
  });
  page.on("pageerror", (error) => errors.page.push(String(error?.message || error)));
  page.on("request", (request) => {
    const raw = request.url() + "\n" + (request.postData() || "");
    try {
      networkSurface.push(decodeURIComponent(raw));
    } catch {
      networkSurface.push(raw);
    }
  });
  page.on("requestfailed", (request) => {
    errors.request.push(
      `${request.method()} ${request.url()} ${request.failure()?.errorText || "failed"}`,
    );
  });
  page.on("response", (response) => {
    if (response.status() >= 400) {
      errors.http.push(`${response.status()} ${response.request().method()} ${response.url()}`);
    }
  });

  try {
    const response = await page.goto(BASE, { waitUntil: "domcontentloaded" });
    assert.equal(response?.status(), 200);
    assert.ok(
      ["127.0.0.1", "localhost", "::1", "[::1]"].includes(new URL(response.url()).hostname),
    );
    for (const agent of ["Claude Code", "Grok", "Codex"]) {
      await page.getByRole("heading", { name: agent, exact: true }).waitFor();
    }
    await page.getByRole("button", { name: "重置演示" }).waitFor();
    await page.getByText("当前是演示数据。", { exact: false }).waitFor();
    const pause = page.getByRole("button", { name: "全部暂停" });
    await pause.click();
    await page.getByRole("button", { name: "开始协同" }).waitFor();
    await page.waitForTimeout(1500);

    const state = await page.evaluate(() => {
      const raw = localStorage.getItem("synq-quota-v8");
      return raw ? JSON.parse(raw)?.state : null;
    });
    assert.equal(state?.demoMode, true);
    assert.equal(state?.onboardingComplete, true);
    assert.equal(availabilityIntercepted, true);
    const claudeCard = cardAround(page, "Claude Code");
    for (const text of [
      "5 小时窗（官方）",
      "本周额度（官方）",
      "Fable 5 周额度（官方）",
      "24%",
      "34%",
      "26%",
    ]) {
      await claudeCard.getByText(text, { exact: true }).first().waitFor();
    }
    await cardAround(page, "Grok").getByText("37%", { exact: true }).first().waitFor();
    await cardAround(page, "Codex").getByText("32%", { exact: true }).first().waitFor();
    const surface = await page.evaluate(() => {
      const attributes = [...document.querySelectorAll("*")]
        .flatMap((element) => [...element.attributes].map((attribute) => attribute.value))
        .join("\n");
      return (
        document.body.innerText +
        "\n" +
        attributes +
        "\n" +
        (localStorage.getItem("synq-quota-v8") || "")
      );
    });
    assertPrivateTextAbsent(surface + "\n" + networkSurface.join("\n"));
    const metrics = await page.evaluate(() => ({
      innerWidth: window.innerWidth,
      scrollWidth: document.documentElement.scrollWidth,
      clientWidth: document.documentElement.clientWidth,
    }));
    assert.equal(metrics.innerWidth, spec.viewport.width);
    assert.ok(metrics.scrollWidth <= metrics.clientWidth, JSON.stringify(metrics));
    assert.deepEqual(errors, { console: [], page: [], request: [], http: [] });

    const path = checkedOutputPath(resolve(OUTPUT_DIR, spec.file), OUTPUT_ROOTS);
    mkdirSync(dirname(path), { recursive: true });
    await page.screenshot({ path, fullPage: true });
    const size = pngSize(path);
    assert.equal(size.width, spec.viewport.width);
    assert.ok(size.height > spec.viewport.height, JSON.stringify(size));
    assert.ok(size.height <= spec.maxHeight, JSON.stringify(size));
    return { name: spec.name, path, size, metrics, errors };
  } finally {
    await context.close();
  }
}

assertReadmeContract();
const browser = await chromium.launch({
  headless: true,
  channel: "chrome",
  args: ["--no-sandbox", "--disable-dev-shm-usage"],
});
const results = [];
try {
  for (const spec of SPECS) results.push(await capture(browser, spec));
} finally {
  await browser.close();
}
writeFileSync(
  "/tmp/synq-public-screenshots.json",
  `${JSON.stringify({ base: BASE, results }, null, 2)}\n`,
);
console.log(JSON.stringify({ base: BASE, results }, null, 2));
```

脚本先通过 `/Applications/Synq.app` 的 `127.0.0.1:4780` 运行：

```bash
npm run screenshots:public
```

绿灯：重新运行 Step 1 的 `git diff HEAD` 断言必须退出 0；`file`/`sips` 必须确认两张图是可解码 PNG，宽度分别为 1280 和 390，高度在脚本硬上限内。

## Step 3：隐私、视觉、真实 App 与 production 回归

先证明 4780 的监听者确实来自安装版 App，而不是同端口替身：

```bash
test -n "$(ps -Ao command= | awk '$1 == "/Applications/Synq.app/Contents/MacOS/synq-desktop" { print }')"
test -n "$(ps -Ao command= | awk '$1 == "/Applications/Synq.app/Contents/MacOS/synq-node" { print }')"
lsof -nP -iTCP:4780 -sTCP:LISTEN | grep '127.0.0.1:4780 (LISTEN)'
test "$(curl --noproxy '*' -fsS --max-time 2 http://127.0.0.1:4780/api/desktop-health)" = \
  '{"app":"synq","mode":"desktop"}'
```

用 `view_image` 逐张查看原始分辨率，确认只有合成任务/泛化路径，三张 Agent 卡、额度来源、时间线、建议、图表和实时流水完整，mobile 无裁切、重叠或不可读区域。

运行仓库门禁，并由同一个 wrapper 为每条命令单独记录退出码与 drain sentinel；wrapper 会跑完三条命令后再统一失败，避免首个 `exit` 截断后续门禁：

```bash
set -e
gate_failed=0
run_gate() {
  gate_label=$1
  shift
  gate_log="/tmp/synq-public-${gate_label}.log"
  gate_exit="/tmp/synq-public-${gate_label}.exit"
  set +e
  "$@" >"$gate_log" 2>&1
  result_code=$?
  set -e
  printf '%s\n' "$result_code" >"$gate_exit"
  tail -n 120 "$gate_log"
  printf '===DRAIN-SENTINEL:%s===\n' "$gate_label"
  if [ "$result_code" -ne 0 ]; then gate_failed=1; fi
}
run_gate npm-test npm test
run_gate typecheck npm run typecheck
run_gate build npm run build
test "$gate_failed" -eq 0
```

默认 `npm run build` 使用 Vercel preset，只作为部署构建门禁，不假设它能由 `nitro preview` 启动。另运行 `npm run build:desktop:web` 生成明确的 `node-server` 生产产物，再用相同截图脚本验证；输出写入 `/tmp`，不再次修改 tracked 截图：

```bash
set -eu
npm run build:desktop:web >/tmp/synq-public-node-server-build.log 2>&1
npx nitro preview --host 127.0.0.1 --port 8081 >/tmp/synq-public-preview.log 2>&1 &
preview_pid=$!
trap 'kill "$preview_pid" 2>/dev/null || true; wait "$preview_pid" 2>/dev/null || true' EXIT INT TERM
attempt=0
until curl --noproxy '*' -fsS --max-time 2 http://127.0.0.1:8081/ >/dev/null; do
  attempt=$((attempt + 1))
  if [ "$attempt" -ge 60 ]; then
    tail -n 120 /tmp/synq-public-preview.log
    exit 1
  fi
  sleep 0.25
done
node scripts/capture-public-screenshots.mjs \
  http://127.0.0.1:8081/ \
  /tmp/synq-public-production
kill "$preview_pid" 2>/dev/null || true
wait "$preview_pid" 2>/dev/null || true
trap - EXIT INT TERM
```

验收：安装版和 production 两条路径各通过 desktop/mobile；`/tmp/synq-public-screenshots.json` 中所有错误数组为空；`node --test scripts/repository-hygiene.test.mjs` 通过；三条 exit 文件均为 `0`。

## Step 4：显式暂存并提交

只暂存以下五个文件，禁止 `git add .`：

```bash
git add -- \
  docs/plans/2026-08-20-refresh-public-screenshots.md \
  package.json \
  scripts/capture-public-screenshots.mjs \
  screenshots/claude-grok-quota-desktop.png \
  screenshots/claude-grok-quota-mobile.png
```

提交前运行 `git diff --cached --check`，并用下面的精确集合断言阻止任何既有用户文件混入：

```bash
expected=$(printf '%s\n' \
  docs/plans/2026-08-20-refresh-public-screenshots.md \
  package.json \
  screenshots/claude-grok-quota-desktop.png \
  screenshots/claude-grok-quota-mobile.png \
  scripts/capture-public-screenshots.mjs | LC_ALL=C sort)
actual=$(git diff --cached --name-only | LC_ALL=C sort)
test "$actual" = "$expected"
git diff --cached --stat
git status --short
```

提交信息：

```text
docs: refresh public app screenshots

Verified-by: npm test (274 passed), npm run typecheck, npm run build, public screenshot E2E (app+prod, 2 viewports)
```

实际提交时将测试计数写为 `274 passed`。提交后运行 `git show --stat --oneline HEAD`、`git log --oneline -3`、`git status --short`，确认 commit 已落盘且既有用户改动仍未暂存。不 push；用户本轮只授权本地 commit。

## 执行结果

- 安装版截图 E2E：desktop `1280×2466`、mobile `390×4805`；两者无横向溢出，console/page/request/HTTP 错误数组全空。
- node-server 生产产物截图 E2E：desktop `1280×2466`、mobile `390×4805`；错误数组全空。
- `npm test`：`274 passed; 0 failed`。
- `node --test scripts/repository-hygiene.test.mjs`：`4 passed; 0 failed`。
- `npm run typecheck`、`npm run build`、`node --check scripts/capture-public-screenshots.mjs`：退出码均为 `0`。
- `npm run lint`：退出码 `0`；全仓保留 6 个既有 warning，本次脚本无 lint error/warning。

## 四关自检

- **Spec coverage**：覆盖 README 精确图片集合、匿名演示、desktop/mobile、安装版身份、production、隐私扫描、视觉检查、显式 stage 和 commit。
- **Placeholder scan**：脚本与命令完整；commit trailer 的动态测试计数只能来自真实输出，不是实现占位符。
- **Type consistency**：使用已核对的 Playwright、浏览器 guard 和 DOM API；不修改应用公共类型。
- **Step size**：红灯、实现/生成、真实验收、提交可独立验证，且不跳步。
