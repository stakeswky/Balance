...
3. 卡片主数字使用 5 小时与每周窗口中“已用百分比最高”的一组，也就是当前最紧的官方额度；简约模式、极客模式与顶部摘要必须共用同一选择规则。
...
- Antigravity 是“官方余量型”卡片：没有日志采集开关、可手选套餐、token 图表、模型占比和 API 等价金额。
...
4. 浏览器 E2E 在 Antigravity-only fixture 下验证简约模式、极客模式与顶部摘要的主剩余百分比一致，极客模式显示四个额度池，不显示采集按钮和 API 等价金额。
