...
const sourceQuotaDir = fileURLToPath(new URL("../../../../src/lib/quota/", import.meta.url));
const tempRoot = await mkdtemp(join(tmpdir(), "synq-verify-band-"));
const patchedQuotaDir = join(tempRoot, "quota");
const quotaValuePath = join(patchedQuotaDir, "quota-value.ts");
let quotaValueModule;

try {
  await cp(sourceQuotaDir, patchedQuotaDir, { recursive: true });
  const source = await readFile(quotaValuePath, "utf8");
  const before = [
  ...
  if (!source.includes(before)) throw new Error("quota band patch target was not found");
  await writeFile(quotaValuePath, source.replace(before, after));
  quotaValueModule = await import(pathToFileURL(quotaValuePath).href);
} finally {
  await rm(tempRoot, { recursive: true, force: true });
}

const { calibrateFromSamples } = quotaValueModule;
