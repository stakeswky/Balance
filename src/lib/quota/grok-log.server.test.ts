...
describe("bounded scanner response (Grok)", () => {
  let tmpDir: string;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "grok-bounded-test-"));
  });
  afterEach(() => {
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });
  ...
  it("since>0 returns only overlap events and quotaCacheEvents has all", () => {
    ...
    const lines: string[] = [];
    for (let i = 0; i < 20; i++) {
      lines.push(grokUpdateLine(baseTs + i * 10_000, i));
    }
    fs.writeFileSync(updatesPath, lines.map((l) => l + "\n").join(""));

    const state1 = createGrokScanState();
    const full = scanGrokUsage(0, { home, now, state: state1 });
    assert.ok(full.events.length > 0, "full scan should yield events");

    const since = baseTs + 10 * 10_000;
    const state2 = createGrokScanState();
    const bounded = scanGrokUsage(since, { home, now, state: state2 });
  ...
  it("all response events carry 64-char hex cacheIdentity", () => {
    ...
    const lines: string[] = [];
    for (let i = 0; i < 5; i++) {
      lines.push(grokUpdateLine(baseTs + i * 1000, i));
    }
    fs.writeFileSync(updatesPath, lines.map((l) => l + "\n").join(""));

    const state = createGrokScanState();
    const result = scanGrokUsage(0, { home, now, state });
